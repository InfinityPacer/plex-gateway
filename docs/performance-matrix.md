...
2026-09-04 的 `control-v1` 候选验收使用 ShimWeave 0.0.1。MKV、HEVC、AAC 样本已完成
从头播放、跳转到 600 秒后继续播放和关闭播放；本地媒体保持 Plex 原路径。观测到的 617 次
Gateway 控制请求全部返回预期的 `204`，处理失败为 0。该结果验证的是 Gateway 控制协议和
CDN 直达边界，不代表浏览器可解码所有 4K、HDR、Dolby Vision 或音频组合。
...
包括本机代码基准、5/5 成功的 MediaVault/CDN 探测抽样、长剧集和本地媒体 A/B，以及
ShimWeave `control-v1` 的目标 NAS 真实播放。尚未覆盖的格式或环境必须继续单独验收，不能从
一个成功样本外推为普遍兼容。
