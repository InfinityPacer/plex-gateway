...
func TestMetadataGuardOnlyMatchesSingleItemReads(t *testing.T) {
	tests := []struct {
		method string
		path   string
		want   bool
	}{
		{method: http.MethodGet, path: "/library/metadata/42", want: true},
		{method: http.MethodHead, path: "/library/metadata/42?includeMarkers=1", want: true},
		{method: http.MethodGet, path: "/library/metadata/42/children", want: false},
		{method: http.MethodGet, path: "/library/metadata/42,43", want: false},
		{method: http.MethodGet, path: "/library/metadata/", want: false},
		{method: http.MethodPost, path: "/library/metadata/42", want: false},
		{method: http.MethodGet, path: "/library/parts/42/1/file", want: false},
		{method: http.MethodGet, path: "/:/timeline", want: false},
	}
	for _, test := range tests {
		t.Run(test.method+" "+test.path, func(t *testing.T) {
			request := httptest.NewRequest(test.method, test.path, nil)
			if got := isDetailedMetadataRequest(request); got != test.want {
				t.Fatalf("isDetailedMetadataRequest() = %t, want %t", got, test.want)
			}
		})
	}
}
